...
function ProfileCompletionCard({ profile }) {
  ...
  let filledCount = 0;
  if (profile) {
    if (checkFilled(profile.trade)) filledCount++;
    if (checkFilled(profile.city)) filledCount++;
    if (checkFilled(profile.state)) filledCount++;
    if (checkFilled(profile.experience)) filledCount++;
    if (checkFilled(profile.bio)) filledCount++;
    if (checkFilled(profile.skills)) filledCount++;
